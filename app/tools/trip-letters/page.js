...
const TRIP_TYPES = [
  { label: '🏛️ Museum', desc: 'Museum or gallery visit' },
  { label: '🌿 Outdoor', desc: 'Outdoor or nature trip' },
  { label: '🎭 Theatre', desc: 'Theatre or performance' },
  { label: '⚽ Sports', desc: 'Sports event or fixture' },
  { label: '🔬 Science', desc: 'Science centre or lab' },
  { label: '🌍 Residential', desc: 'Overnight or residential' },
  { label: '🏫 Campus', desc: 'University or college visit' },
  { label: '📋 Other', desc: 'Other educational visit' },
]

const REFINE_OPTIONS = [
  { label: '📐 More formal', prompt: 'Make this letter more formal' },
  { label: '☀️ Friendlier', prompt: 'Make this letter friendlier and more welcoming' },
  { label: '✂️ Shorter', prompt: 'Make this letter shorter and more concise' },
  { label: '📖 More detailed', prompt: 'Add more detail to this letter' },
]

export default function TripLetters() {
  const [form, setForm] = useState({
    yearGroup: '', tripType: '', destination: '', date: '', cost: '', notes: ''
  })
  ...
  const handleGenerate = async (refinePrompt = null) => {
    if (!form.yearGroup || !form.tripType || !form.destination) return
    setLoading(true)
    setResult('')
    try {
    ...
    <main style={{minHeight:'100vh', background:'#FAF7F2', fontFamily:'Inter, system-ui, sans-serif'}}>
      ...
      <div className="container">
        ...
        <div className="tool-header">
          ...
          <h1 className="tool-title">Trip Letters</h1>
          <p className="tool-desc">All the information parents need, none of the faff. Ready to send home same day.</p>
        </div>
        ...
        <div className="card">
          <div className="row">
            ...
            <div className="field">
              ...
              <input
                type="text"
                placeholder="e.g. Natural History Museum, London"
          ...
          <div className="field">
            ...
            <div className="type-grid">
              {TRIP_TYPES.map(t => (
                <button
                  key={t.label}
                  className={`type-btn ${form.tripType === t.label ? 'active' : ''}`}
                  onClick={() => setForm({...form, tripType: t.label})}
                  type="button"
                >
                  {t.label}
          ...
          <div className="row">
            <div className="field">
              <label>Date <span style={{fontWeight:400, textTransform:'none', fontSize:'0.75rem'}}>(optional)</span></label>
              <input
                type="text"
            ...
            <div className="field">
              <label>Cost <span style={{fontWeight:400, textTransform:'none', fontSize:'0.75rem'}}>(optional)</span></label>
              <input
                type="text"
          ...
          <div className="field">
            ...
            <textarea
              placeholder="e.g. Students need packed lunch, wear PE kit, meet at school gates at 8:45am, back by 4pm."
              value={form.notes}
          ...
          <button
            ...
            disabled={loading || !form.yearGroup || !form.tripType || !form.destination}
          >
