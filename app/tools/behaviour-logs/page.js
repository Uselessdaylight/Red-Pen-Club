...
const INCIDENT_TYPES = [
  { label: '😤 Disruption', desc: 'Disrupting the class' },
  { label: '🤬 Verbal', desc: 'Verbal altercation' },
  { label: '👊 Physical', desc: 'Physical incident' },
  { label: '📵 Device', desc: 'Unauthorised device use' },
  { label: '🚪 Truancy', desc: 'Leaving without permission' },
  { label: '😔 Defiance', desc: 'Refusing to follow instructions' },
  { label: '🎭 Other', desc: 'Other behaviour concern' },
]

const REFINE_OPTIONS = [
  { label: '📐 More formal', prompt: 'Make this log more formal and clinical' },
  { label: '📋 More detailed', prompt: 'Add more detail and context to this log' },
  { label: '✂️ More concise', prompt: 'Make this log shorter and more concise' },
  { label: '⚖️ More neutral', prompt: 'Make this log more neutral and factual, removing any emotive language' },
]

export default function BehaviourLogs() {
  const [form, setForm] = useState({
    yearGroup: '', incidentType: '', location: '', notes: ''
  })
  ...
  const handleGenerate = async (refinePrompt = null) => {
    if (!form.yearGroup || !form.incidentType || !form.notes) return
    setLoading(true)
    setResult('')
    try {
      const res = await fetch('/api/generate-behaviour', {
        method: 'POST',
    ...
    <main style={{minHeight:'100vh', background:'#FAF7F2', fontFamily:'Inter, system-ui, sans-serif'}}>
      ...
      <div className="container">
        ...
        <div className="tool-header">
          ...
          <h1 className="tool-title">Behaviour Logs</h1>
          <p className="tool-desc">Clear, factual, professional incident documentation — ready in seconds.</p>
        </div>
        ...
        <div className="card">
          <div className="row">
            ...
            <div className="field">
              <label>Location <span style={{fontWeight:400, textTransform:'none', fontSize:'0.75rem'}}>(optional)</span></label>
              <input
                type="text"
                placeholder="e.g. Classroom 4B, Corridor, Hall"
                value={form.location}
          ...
          <div className="field">
            ...
            <div className="type-grid">
              {INCIDENT_TYPES.map(t => (
                <button
                  key={t.label}
                  className={`type-btn ${form.incidentType === t.label ? 'active' : ''}`}
          ...
          <div className="field">
            ...
            <textarea
              placeholder="e.g. Student refused to put phone away after three requests, became verbally aggressive when asked again, used inappropriate language towards staff."
              value={form.notes}
          ...
          <button
            ...
            disabled={loading || !form.yearGroup || !form.incidentType || !form.notes}
          >
            {loading ? 'Documenting...' : '✦ Generate log'}
          </button>
        </div>
        ...
        {result && !loading && (
          <div className="card">
            <div className="output-header">
              <div className="output-label">✦ Your log</div>
            ...
            <div className="refine-label">Refine this log</div>
