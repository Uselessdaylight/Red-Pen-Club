...
const TARGET_TYPES = [
  { label: '📈 Academic', desc: 'Skill or knowledge target' },
  { label: '🎯 Effort', desc: 'Attitude or work ethic' },
  { label: '🤝 Behaviour', desc: 'Conduct or attitude' },
  { label: '📝 Homework', desc: 'Completion or quality' },
  { label: '🗣️ Participation', desc: 'Engagement in class' },
  { label: '📊 Exam prep', desc: 'Revision or technique' },
]

const REFINE_OPTIONS = [
  { label: '🎯 More specific', prompt: 'Make this target more specific and measurable' },
  { label: '☀️ More encouraging', prompt: 'Make this target more encouraging and positive' },
  { label: '✂️ Shorter', prompt: 'Make this target shorter and more concise' },
  { label: '📖 More detailed', prompt: 'Make this target more detailed with clearer steps' },
]

export default function StudentTargets() {
  const [form, setForm] = useState({
    yearGroup: '', subject: '', targetType: '', notes: ''
  })
  ...
  const handleGenerate = async (refinePrompt = null) => {
    if (!form.yearGroup || !form.subject || !form.targetType || !form.notes) return
    setLoading(true)
    setResult('')
    try {
    ...
    <main style={{minHeight:'100vh', background:'#FAF7F2', fontFamily:'Inter, system-ui, sans-serif'}}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 480px) { .row { grid-template-columns: 1fr; } .subject-grid { grid-template-columns: repeat(3, 1fr); } .type-grid { grid-template-columns: repeat(2, 1fr); } }
      `}</style>
      ...
      <div className="container">
        ...
        <div className="tool-header">
          ...
          <h1 className="tool-title">Student Targets</h1>
          <p className="tool-desc">Specific, actionable next steps tailored to each student. No more generic "try harder."</p>
        </div>
        ...
        <div className="card">
          ...
          <div className="field">
            ...
            <div className="type-grid">
              {TARGET_TYPES.map(t => (
                <button
                  key={t.label}
                  className={`type-btn ${form.targetType === t.label ? 'active' : ''}`}
                  onClick={() => setForm({...form, targetType: t.label})}
                  type="button"
                >
                  {t.label}
          ...
          <div className="field">
            ...
            <textarea
              placeholder="e.g. Struggles to show working out in maths. Gets the right answers but can't explain method. Needs to develop written reasoning skills."
              value={form.notes}
          ...
          <button
            ...
            disabled={loading || !form.yearGroup || !form.subject || !form.targetType || !form.notes}
          >
            {loading ? 'Setting target...' : '✦ Generate target'}
          </button>
        </div>
        ...
        {result && !loading && (
          <div className="card">
            <div className="output-header">
              <div className="output-label">✦ Your target</div>
            ...
            <div className="refine-label">Refine this target</div>
