...
const ARTICLE_TYPES = [
  { label: '🏆 Achievement', desc: 'Celebrating success' },
  { label: '📅 Upcoming event', desc: 'What\'s coming up' },
  { label: '📸 Trip recap', desc: 'Reviewing a visit' },
  { label: '🎨 Project update', desc: 'Class project news' },
  { label: '📢 Reminder', desc: 'Important reminder' },
  { label: '👋 Introduction', desc: 'Meet the teacher' },
  { label: '🌟 Term update', desc: 'End of term roundup' },
  { label: '📋 Other', desc: 'General article' },
]

const REFINE_OPTIONS = [
  { label: '☀️ Warmer', prompt: 'Make this article warmer and more personal' },
  { label: '✂️ Shorter', prompt: 'Make this article shorter and more concise' },
  { label: '📖 Longer', prompt: 'Make this article longer and more detailed' },
  { label: '🎉 More upbeat', prompt: 'Make this article more upbeat and celebratory' },
]

export default function NewsletterArticles() {
  const [form, setForm] = useState({
    yearGroup: '', articleType: '', notes: ''
  })
  ...
  const handleGenerate = async (refinePrompt = null) => {
    if (!form.yearGroup || !form.articleType || !form.notes) return
    setLoading(true)
    setResult('')
    try {
    ...
    <main style={{minHeight:'100vh', background:'#FAF7F2', fontFamily:'Inter, system-ui, sans-serif'}}>
      ...
      <div className="container">
        ...
        <div className="tool-header">
          ...
          <h1 className="tool-title">Newsletter Articles</h1>
          <p className="tool-desc">Keep parents informed without it taking your whole lunch break. Just add the key points.</p>
        </div>
        ...
        <div className="card">
          ...
          <div className="field">
            ...
            <div className="type-grid">
              {ARTICLE_TYPES.map(t => (
                <button
                  key={t.label}
                  className={`type-btn ${form.articleType === t.label ? 'active' : ''}`}
                  onClick={() => setForm({...form, articleType: t.label})}
                  type="button"
                >
                  {t.label}
          ...
          <div className="field">
            ...
            <textarea
              placeholder="e.g. Year 4 won the school spelling bee this week. Three students made the final — Emma, Jayden and Priya. Emma won overall with a perfect score. Very proud of the whole class."
              value={form.notes}
          ...
          <button
            ...
            disabled={loading || !form.yearGroup || !form.articleType || !form.notes}
          >
        ...
        {result && !loading && (
          <div className="card">
            <div className="output-header">
              <div className="output-label">✦ Your article</div>
            ...
            <div className="refine-label">Refine this article</div>
